...
PAGE_TITLE="ValueMart | Retail Analytics"
ACCENT="#0f7550"; ACCENT_DARK="#074933"; ACCENT2="#bd704e"; SOFT="#e9f4ef"
OVERVIEW_URL="https://una471.github.io/portfolio/projects/retail/overview.html"; OVERVIEW_LABEL="Sales Overview"; SOFTWARE_URL="https://retail-system.streamlit.app/?view=software"; 
...
def load_data():
    ...
    inv=pd.read_csv(ROOT/"inventory_levels.csv")
    promo=pd.read_csv(ROOT/"promo_effectiveness.csv")
    cat=pd.read_csv(ROOT/"category_performance.csv")
    rules=pd.read_csv(ROOT/"association_rules.csv")
    return tx,inv,promo,cat,rules
tx,inv,promo,cat,rules=load_data()
with st.sidebar:
    sidebar_brand("VM", "VALUEMART", "Retail Intelligence")
    page=st.radio("Go to",["Sales Overview","Market Basket","Promotions","Inventory Alerts"],index=0,label_visibility="collapsed")
    st.divider(); cats=["All Categories"]+sorted(tx.category.dropna().unique().tolist()); promos=["All Promotions"]+sorted(tx.promo_period.dropna().unique().tolist()); cat_sel=st.selectbox("Category",cats,index=0); promo_sel=st.selectbox("Promotion",promos,index=0)
    st.divider(); st.caption("Period: Jan to Dec 2025"); st.markdown('<div class="side-promo"><div><strong>Fresh products. Stronger communities.</strong><span>Sales, stock and promotion decisions in one clear view.</span></div></div>',unsafe_allow_html=True)
df=tx.copy()
if cat_sel!="All Categories": df=df[df.category==cat_sel]
if promo_sel!="All Promotions": df=df[df.promo_period==promo_sel]
search_query=utilitybar("Retail Analyst")
df=apply_search(df,search_query)

def money(x): return f"P{x/1e6:.2f}M" if abs(x)>=1e6 else f"P{x/1e3:.0f}K" if abs(x)>=1e3 else f"P{x:,.2f}"

def revenue_series(frame):
    x=frame.assign(revenue=frame.final_price*frame.quantity); return x

def sales_overview():
    hero("Retail Performance","Sales, customers, inventory and promotion performance across ValueMart.")
    x=revenue_series(df); rev=x.revenue.sum(); transactions=x.transaction_id.nunique(); units=x.quantity.sum(); basket=rev/max(transactions,1); stockrisk=int(inv.stockout_risk.sum())
    cols=st.columns(6); vals=[("Revenue",money(rev),"Selected view"),("Transactions",f"{transactions:,}","Customer baskets"),("Average Basket",money(basket),"Revenue / transaction"),("Units Sold",f"{units/1e3:.1f}K","Selected view"),("Stockout Risk",f"{stockrisk}","Products flagged"),("Categories",f"{x.category.nunique()}","Active categories")]
    for i,v in enumerate(vals): cols[i].markdown(kcard(*v,primary=i==0),unsafe_allow_html=True)
    c1,c2,c3=st.columns([1.2,1.2,1])
    with c1:
        panel_head("Revenue by Category","Contribution to selected revenue")
        q=x.groupby('category').revenue.sum().sort_values().reset_index(); fig=px.bar(q,x='revenue',y='category',orientation='h',color_discrete_sequence=[ACCENT]); st.plotly_chart(chart_style(fig,255,False),use_container_width=True)
    with c2:
        panel_head("Monthly Revenue Trend","Seasonality and trading momentum")
        q=x.assign(month=x.date.dt.to_period('M').astype(str)).groupby('month').revenue.sum().reset_index(); fig=px.line(q,x='month',y='revenue',markers=True,color_discrete_sequence=[ACCENT]); st.plotly_chart(chart_style(fig,255,False),use_container_width=True)
    with c3:
        panel_head("Promotion Effectiveness","Sales lift by campaign")
        q=promo.sort_values('sales_lift_pct'); fig=px.bar(q,x='sales_lift_pct',y='promo_period',orientation='h',color_discrete_sequence=[ACCENT2]); st.plotly_chart(chart_style(fig,255,False),use_container_width=True)
    st.markdown('<div class="section-title">Top Performing Products</div>',unsafe_allow_html=True)
    q=x.groupby(['product','category']).agg(units=('quantity','sum'),revenue=('revenue','sum')).reset_index().sort_values('revenue',ascending=False).head(15); st.dataframe(q,use_container_width=True,hide_index=True,height=315)

def basket_page():
    hero("Market Basket Analysis","Understand which products are commonly purchased together and where cross-sell opportunities exist.")
    st.dataframe(rules.sort_values(['lift','confidence'],ascending=False),use_container_width=True,hide_index=True,height=480)

def promo_page():
    hero("Promotion Performance","Compare campaign lift, revenue and transaction response.")
    c1,c2=st.columns(2)
    with c1:
        fig=px.bar(promo,x='promo_period',y='sales_lift_pct',color_discrete_sequence=[ACCENT]); st.plotly_chart(chart_style(fig,320,False),use_container_width=True)
    with c2:
        fig=px.scatter(promo,x='transactions',y='daily_revenue',size='days',text='promo_period',color_discrete_sequence=[ACCENT2]); st.plotly_chart(chart_style(fig,320,False),use_container_width=True)
    st.dataframe(promo,use_container_width=True,hide_index=True)

def inventory_page():
    hero("Inventory Alerts","Identify products with limited cover and prioritize replenishment.")
    q=inv.sort_values(['stockout_risk','days_of_stock'],ascending=[False,True]); st.dataframe(q,use_container_width=True,hide_index=True,height=520)

{"Sales Overview":sales_overview,"Market Basket":basket_page,"Promotions":promo_page,"Inventory Alerts":inventory_page}[page]()
